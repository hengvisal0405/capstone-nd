...
from app.base.ionepy.translate import get_translation, set_lang
...
async def paginate_sql(db: AsyncSession, full_sql: str, params: Dict,
                       page: int, page_size: int):
    offset = (page - 1) * page_size
    count_sql = f"SELECT COUNT(*) FROM ({full_sql}) AS count_subquery"
    count_result = await db.execute(text(count_sql), params)
    total_records = count_result.scalar_one()
    paginated_sql = f"{full_sql} ORDER BY id OFFSET :offset LIMIT :limit"
...
async def paginate(db: AsyncSession,
                   ...
                   filters: List[Any],
                   page: int = 1,
                   page_size: int = 10) -> Tuple[List[Any], int, int]:
    """
...
def app_success_paginated(
    ...
    if all(v is not None for v in
           [total_records, total_pages, current_page, page_size, lists]):
        base_data = {
...
def get_phnom_penh_time_formatted(dt: datetime) -> str:
    """
    Format a datetime object to 'Month Day Year' (e.g., 'July 05 2025')
    """
...
async def check_existing_record(db: AsyncSession, table_name: str,
                                id_value: Optional[int], translate: str):
    """
...
def set_ordering(model):
    ...
    def assign_ordering(mapper, connection, target):
        connection.execute(model.__table__.update().where(
            model.id == target.id).values(ordering=target.id))


def export_file_response(file: BytesIO, filename: str,
...
async def protect_route(
    ...
    if current_path not in allowed_routes:
        raise AppException(code=403,
                           msg=get_translation("unautherized_access"))
    return employee


async def get_language(lang: Optional[int] = None):
    """"
